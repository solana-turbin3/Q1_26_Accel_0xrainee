...
use wincode::{DeserializeOwned as WincodeDeserialize, Serialize as WincodeSerialize};
...
impl<T: WincodeSerialize<Src = T> + WincodeDeserialize<Dst = T>> Serializer<T>
    for WincodeSerializer
{
    fn to_bytes(&self, value: &T) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
